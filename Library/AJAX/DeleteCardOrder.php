...
$statement = "SELECT * FROM " . $DB_TablePrefix . "KARTENBESTELLUNG WHERE ID = " . $jobid;

if (count(execSelect($statement)) != 1) {
    $response = array(
        "success" => false,
        "message" => "Bestellung '" . $jobid . "' wurde nicht gefunden!",
        "id" => $jobid
    );
...
$delete = "DELETE FROM " . $DB_TablePrefix . "KARTENBESTELLUNG WHERE Id = :id";
...
if ($success && count(execSelect($statement)) == 0) {
    $response = array(
        "success" => true,
        "message" => "Bestellung '" . $jobid . "' erfolgreich gelöscht!",
        "id" => $jobid
    );
} else {
    $e = oci_error($conn);
